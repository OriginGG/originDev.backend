...
DROP TYPE IF EXISTS  origin.auth_payload;
DROP TYPE IF EXISTS  origin.jwt_token; 
DROP TYPE IF EXISTS  origin.pre_jwt_token; 
...
begin
  select a.id, a.admin_user, a.password_hash, a.email, a.organisation_id into account 
  from origin.users as a
  where a.email = $1;
  if account.password_hash = crypt(password, account.password_hash) then
		if account.admin_user = false then
    	return (('origin_user', account.id, account.organisation_id)::origin.jwt_token, account.id, account.organisation_id, false);
		else 
    	return (('origin_admin', account.id, account.organisation_id)::origin.jwt_token,  account.id, account.organisation_id, true);
		end if;
	else
    return null;
  end if;
...
declare
  person origin.users;
begin
  insert into origin.users (first_name, last_name, organisation_id, email, password_hash, admin_user) values
